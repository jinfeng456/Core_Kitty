<template>
  <div class="page-container">
    ...
    <Wt-table :height="350"
              ...
              permsEdit="purchase:warehousein:edit"
              :data="pageResult"
...
import EditWareHouseIn from "./EditWareHouseIn"
export default {
  components: {
    PopupTreeInput,
    KtTableGt,
  ...
  data () {
    return {
      ...
      dialogVisible: false,//明细显示
      dialogVisibles: false,//转入库单显示
  ...
  methods: {
    ...
    synchronize: function (data) {
      if (this.filters.orgcode == null) {
        this.$message({ message: "公司不能为空", type: 'error' })
      }
      else if (this.filters.beginTime == null) {
        this.$message({ message: "开始时间不能为空", type: 'error' })
      }
      else if (this.filters.endTime == null) {
        this.$message({ message: "结束时间不能为空", type: 'error' })
      }
