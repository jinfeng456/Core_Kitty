  ...
  <div>
    ...
    <el-table :data="data.content"
      ...
      <el-table-column type="selection"
                       width="40"
                       v-if="showBatchEdit & showOperation"></el-table-column>
      <!-- 序号 -->
    ...
    <div class="toolbar"
         style="padding:10px;">
      <kt-button :label="$t('action.batchEdit')"
                 :perms="permsBatchEdit"
                 :size="size"
                 type="danger"
                 @click="handleBatchEdit()"
                 :disabled="this.selections.length===0"
                 style="float:left;"
                 v-if="showBatchEdit & showOperation" />
...
export default {
  ...
  methods: {
    ...
    // 批量编辑
    handleBatchEdit: function (index, row) {
      let ids = this.selections.map(item => item.id).toString()
      this.$emit('handleBatchEdit', { index: index, row: row })
      this.edit(ids);
      //alert("走着");
    },
    // 删除
    ...
    edit: function (ids) {
      ...
      this.$emit('handleBatchEdit', { params: params, callback: callback })
      // }).catch(() => {
      // })
    }
  },
