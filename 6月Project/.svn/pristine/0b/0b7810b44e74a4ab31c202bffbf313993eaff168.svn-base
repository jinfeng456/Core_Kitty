  ...
  methods: {
    ...
    ImportExcelData(excelData){  
                  ...
                  excelData.forEach(item => {
                      let obj = {};
                      debugger
                      obj.itemId = this.itemToId(item["物料名称"]);
                      obj.supplierId = this.supplierToId(item["供应商名称"]);                           
                      arr.push(obj);  
                      this.percentageValue= Math.round(this.percentageValue+100/excelData.length) 
                 
                  });
                   this.$api.corewhgroitem.ImportGcList(arr).then((res) => {
                        if(res.code==200){
    ...
    //导出的方法
		exportExcel() {
        require.ensure([], () => {  
              this.$api.corewhgroitem.GetExportGcList(this.filters).then((res) => {
              // Excel的表格第一行的标题			
              const tHeader = ['物料名称', '供应商名称'];
              // index、nickName、name是tableData里对象的属性
              const filterVal = ['name', 'gysName'];            
              const list = res.data;  //把data里的tableData存到list
              const data = this.formatJson(filterVal, list);
              export_json_to_excel(tHeader, data, '供应商物料信息导出');
          })    	        
        })
      },
    ...
    exportExample() {
        require.ensure([], () => {  
              ...
              const list = res.data;  //把data里的tableData存到list
              const data = this.formatJson(filterVal, list);
              export_json_to_excel(tHeader, data, '供应商物料模板导出');
          })    	        
        })
      },  
    //批量编辑
    ...
    itemToId: function (name) {     
      let list = this.itemTypes;
    ...
    supplierToId: function (name) {     
      let list = this.groTypes;
      for (let i = 0; i < list.length; i++) {
        if (list[i].name == name) {
          return list[i].id;
        }
