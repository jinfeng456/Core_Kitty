	...
	data() {
		return {
			...
			columns: [
				...
				//{prop:"stockId", label:"库存主表主键", minWidth:130},
				...
				{prop:"bussType", label:"出库类型", minWidth:100,formatter:this.selectionFormat1},
				{prop:"taskType", label:"任务类型", minWidth:100,formatter:this.selectionFormat},
				{prop:"src", label:"起始位置", minWidth:100},
				{prop:"des", label:"目标位置", minWidth:100,formatter:this.selectionFormat},
			...
			detailColumns: [
				...
				{prop:"des", label:"目标位置", minWidth:100},
...
      	},
		selectionFormat1: function (row, column, cellValue, index){
			...
			let val=row[column.property];	
			let dict = this.$store.state.dict.dicts['inType'];
			if(dict==undefined){
					return row[column.property]
			}
			for(let i=0;i<dict.length;i++){
			 ...
			 dict = this.$store.state.dict.dicts['outTypeBatch'];
			if(dict==undefined){
					return row[column.property]
			}
			for(let i=0;i<dict.length;i++){
