  ...
  <div>
    ...
    <el-table :data="data.content" :highlight-current-row="highlightCurrentRow" @selection-change="selectionChange" 
      ...
      <el-table-column :label="$t('action.operation')" width="100" fixed="right" v-if="showOperation" header-align="center" align="center">
        <template slot-scope="scope">
          <kt-button icon="fa fa-trash" :label="showReceipOutId?'选择':'解绑'" :perms="permsSelect"  :size="size" type="danger" @click="handleSelect(scope.$index, scope.row)" />
        </template>
      </el-table-column>
    ...
    <div class="toolbar" style="padding:10px;">
      <kt-button :label="showReceipOutId?'选择':'解绑'" :perms="permsSelect"  :size="size" type="danger" @click="handleBatchSelect()" 
...
export default {
  ...
  methods: {
    ...
    // 选择
        ...
        this.select(row.id+'|'+row.stockDetailId)
		},
		// 批量选择
		handleBatchSelect: function () {
			let ids = this.selections.map(item => item.id+'|'+item.stockDetailId).toString()
			this.select(ids)
    },
