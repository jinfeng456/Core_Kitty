...
//需要一个明暗器来绘制网格；网格不能自己绘制。

/ /

//网格是“gl.buffer”对象的集合，这些对象都是顶点缓冲区

//（保留每个顶点属性）或索引缓冲区（保留

//顶点被渲染）。默认情况下，网格的位置顶点缓冲区称为

//`顶点和称为“三角形”的三角形索引缓冲区。新缓冲区可以是

//使用“addVertexBuffer（）”和“addIndexBuffer（）”添加。两个字符串是

//添加新顶点缓冲区时必需，在

//网格实例和顶点明暗器中glsl属性的名称。 
//
// Example usage:
//
