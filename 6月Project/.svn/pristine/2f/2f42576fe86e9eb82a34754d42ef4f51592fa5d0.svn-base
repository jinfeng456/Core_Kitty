<template>
  <div class="page-container">
	<!--新增编辑界面-->
	<el-dialog :title="operation?$t('action.add'):$t('action.edit')" width="80%" :visible.sync="dialogVisible" :close-on-click-modal="false" @opened="getDetials"  :before-close="handleClose">
		...
		<div v-show="operation">
			<div class="app-container">
				<el-table v-loading="listLoading" :data="list" height="250" border fit highlight-current-row style="width: 100%" size="mini">
	  ...
	  <proorder-select :key="componentKey"  :itemDialogVisible="itemDialogVisible" @handleSelect="handleSelect"    @itemHidden="itemHidden"  :items="this.soidList" :showOrder="this.showOrder"></proorder-select>
  </div>
</template>
...
import ProorderSelect from "./ProorderSelect"
export default {
	name: 'EditReceiptOut',
	components:{
		KtTableList,
...
      	},
		...
		handleSelect:function(data){
			//debugger
		 	this.$api.receipt.saveDetialByProduce({id:this.subDataForm.id,items:data}).then((res) => {			
				if(res.code == 200) {
