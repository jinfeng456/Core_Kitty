 ...
 <div class="container" >
	...
	<div class="toolbar" style="float:left;padding-top:10px;padding-left:15px;">
		<el-form :inline="true" :model="filters" :size="size">
			<el-form-item>
				<el-input v-model="filters.itemName" placeholder="物料名称"></el-input>
			</el-form-item>
			<el-form-item>
			...
			<el-form-item>
				<kt-button :label="$t('action.search')" perms="report:statRealOut:view" type="primary" @click="findPage(null)"/>
			</el-form-item>			
		</el-form>
	</div>
...
export default {
	...
	data(){
		return {
			...
			columns: [
				...
				{ prop: "count", label: "发出数量", minWidth: 100},
	...
	methods: {
		...
		findPage: function(data){
			...
			this.$api.statReal.findPage(this.filters).then((res) => {
		...
		exportExcel() {
			...
			require.ensure([], () => {		
					this.$api.statReal.findPage(this.filters).then((res) => {						
					const tHeader = ['物料编码','物料名称','规格','包装规格','单位','月份','数量','内部批次号','结存', '操作时间'];
					const filterVal = ["itemCode","itemName","modelSpecs","packageSpecs","unit",'month','count', 'wmsBanchNo','remain',"opTime"];				
