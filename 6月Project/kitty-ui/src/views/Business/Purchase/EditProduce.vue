         ...
         <el-row>
          <el-col :span="12">
            <el-form-item label="供应商名称"
              ...
              <search-select :itemlist="this.gysTypes"
                             style="width: 100%;"
                             placeholder="输入框"
                             :state="subDataForm.applicantName"
          ...
          <el-col :span="12">
              <el-form-item label="供应商Id"
                          prop="customId">
              <el-input v-model="subDataForm.applicantId"
                        :auto-complete="off">
              </el-input>
            </el-form-item>
          </el-col>
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="供应商编码"
                          prop="customCode">
              <el-input v-model="subDataForm.applicantCode"
                        :auto-complete="off">
              </el-input>
            </el-form-item>
          </el-col>
...
export default {
  ...
  methods: {
    //编辑后确定
    confirmEdit (row) {
      this.$api.produce.updateDetials(row).then((res) => {
        if (res.code == 200) {
              this.getDetials();
            } else {
    ...
    handleDelete: function (index, row) {
      this.$confirm('确认删除选中记录吗？', '提示', {
          ...
          this.$api.produce.deleteDetial({id: row.id, soId: row.soId}).then((res) => {
            if (res.code == 200) {
                  this.getDetials();
                } else {
                  this.$message({ message: this.$t('action.operateFail') + res.msg, type: 'error' })
    ...
    itemclick: function (data) {
      this.componentKey += 1;
      if (data.length!=0){
        this.subDataForm.applicantName = data.name
        this.subDataForm.applicantId = data.ncId
        this.subDataForm.applicantCode = data.code
        this.subDataForm.fromCorpId = data.id
      }else{
        this.subDataForm.applicantName = ""
        this.subDataForm.applicantId = 0
        this.subDataForm.applicantCode = 0
    ...
    getDetials: function () {
      ...
      this.$api.produce.getDetials(this.subDataForm).then((res) => {
        debugger
        ...
        this.$refs.dataForm.validate((valid) => {
          if (valid) {
            this.$confirm(this.$t('action.isConfirm'), this.$t('action.tips'), {}).then(() => {
              ...
              this.$api.produce.saveProduce(params).then((res) => {
