<template>
  <div class="page-container">
    ...
    <div class="toolbar" style="float:left;padding-top:0px;padding-left:15px;">
        ...
        <el-form-item>
          ...
          <kt-button icon="fa fa-search"
                     ...
                     perms="receiptln:report:flatstore"
                     type="primary"
                     @click="findPage(null)" />
        </el-form-item>  
			</el-form-item>	
   ...
   <el-dialog :title="'编辑平库入库明细'"
        ...
        <el-form-item label="reDetailId"
                      prop="reDetailId"
                      v-if="false">
          <el-input v-model="dataForm.reDetailId"
...
export default {
  ...
  methods: {
    ...
    selectionFormat1: function (row, column, cellValue, index) {
      ...
      let dict = this.$store.state.dict.dicts["flatStorageArea"];
      for (let i = 0; i < dict.length; i++) {
  ...
  mounted () {
    this.columns = [
      ...
      { prop: "wmsBanchNo", label: "内部批号", minWidth: 100 },
       { prop: "areaId", label: "入库库区", minWidth: 100,formatter: this.selectionFormat1},
