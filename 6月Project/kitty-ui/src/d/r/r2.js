...
function rightCarrier(context){
	    ...
	    var xV=xBase+carrierlength/2+stockSize;
	 	  var xH=xBase+parseInt((stockSize+1)/2)
    	
    	
    	
    for(var i=1;i<12;i++){
    	  var yBase =getCraneY(i)-stockSize*1.25;
    	  carrierList.push(new CarrierChain(getRightHid(i),context,xH ,getUpStockY(i),carrierlength))
	...
	function leftCarrier(context){
		    	    	...
		    	    	var xH=xBase+carrierlength/2+parseInt((stockSize+1)/2)
            	  ...
            	  for(var i=1;i<12;i++){
            	  	    carrierList.push(new CarrierChain(getLeftHid(i),context,xH,getUpStockY(i),carrierlength))
